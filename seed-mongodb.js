#!/usr/bin/env node

/**
 * MongoDB Data Seeding Script for LostCity
 * Directly inserts mock data into MongoDB Atlas
 */

const { MongoClient, ObjectId } = require('mongodb');

// MongoDB connection string from application.yml
const MONGO_URI = "mongodb+srv://shashikumarezhil_db_user:CZHLkjeFvHz55vC5@lostcity.1g3rszx.mongodb.net/lostcity?retryWrites=true&w=majority&appName=LostCity";
const DB_NAME = "lostcity";

async function seedDatabase() {
  console.log("🌱 Seeding MongoDB with mock data...\n");
  
  const client = new MongoClient(MONGO_URI);
  
  try {
    // Connect to MongoDB
    await client.connect();
    console.log("✅ Connected to MongoDB Atlas\n");
    
    const db = client.db(DB_NAME);
    
    // Clear existing data (optional - comment out if you want to keep existing data)
    console.log("🗑️  Clearing existing data...");
    await db.collection('users').deleteMany({});
    await db.collection('lost_reports').deleteMany({});
    await db.collection('found_reports').deleteMany({});
    await db.collection('match_requests').deleteMany({});
    console.log("✅ Existing data cleared\n");
    
    // ===== INSERT USERS =====
    console.log("👥 Inserting users...");
    const users = [
      {
        _id: new ObjectId(),
        clerkId: "user_test_john123",
        email: "john.doe@test.com",
        displayName: "John Doe",
        avatarUrl: "https://api.dicebear.com/7.x/avataaars/svg?seed=john",
        bio: "Always losing things 😅",
        phone: "+1234567890",
        role: "USER",
        score: 150,
        coins: 100.0,
        lifetimeEarnings: 150.0,
        lifetimeSpent: 50.0,
        badges: ["First Report", "Helpful Finder"],
        skills: ["Quick Response", "Detailed Descriptions"],
        foundReportsCount: 2,
        lostReportsCount: 3,
        itemsReturnedCount: 1,
        questsCompletedCount: 5,
        createdAt: new Date(),
        updatedAt: new Date(),
        _class: "com.lostcity.model.User"
      },
      {
        _id: new ObjectId(),
        clerkId: "user_test_sarah456",
        email: "sarah.smith@test.com",
        displayName: "Sarah Smith",
        avatarUrl: "https://api.dicebear.com/7.x/avataaars/svg?seed=sarah",
        bio: "Helping the community find their lost items",
        phone: "+1987654321",
        role: "USER",
        score: 200,
        coins: 150.0,
        lifetimeEarnings: 200.0,
        lifetimeSpent: 50.0,
        badges: ["Super Helper", "Quest Master"],
        skills: ["Fast Finder", "Detail Oriented"],
        foundReportsCount: 5,
        lostReportsCount: 1,
        itemsReturnedCount: 3,
        questsCompletedCount: 10,
        createdAt: new Date(),
        updatedAt: new Date(),
        _class: "com.lostcity.model.User"
      },
      {
        _id: new ObjectId(),
        clerkId: "user_test_mike789",
        email: "mike.johnson@test.com",
        displayName: "Mike Johnson",
        avatarUrl: "https://api.dicebear.com/7.x/avataaars/svg?seed=mike",
        bio: "Tech enthusiast and finder",
        phone: "+1122334455",
        role: "USER",
        score: 75,
        coins: 75.0,
        lifetimeEarnings: 100.0,
        lifetimeSpent: 25.0,
        badges: ["Early Adopter"],
        skills: ["Electronics Expert"],
        foundReportsCount: 1,
        lostReportsCount: 2,
        itemsReturnedCount: 1,
        questsCompletedCount: 3,
        createdAt: new Date(),
        updatedAt: new Date(),
        _class: "com.lostcity.model.User"
      }
    ];
    
    await db.collection('users').insertMany(users);
    console.log(`✅ Inserted ${users.length} users\n`);
    
    // Get user references
    const john = users[0];
    const sarah = users[1];
    const mike = users[2];
    
    // ===== INSERT LOST REPORTS =====
    console.log("📋 Inserting lost reports...");
    const lostReports = [
      {
        _id: new ObjectId(),
        reportedBy: { $ref: "users", $id: john._id },
        title: "MacBook Pro 16-inch",
        category: "ELECTRONICS",
        description: "Silver MacBook Pro with Lost City stickers on the cover. Has a small dent on the bottom left corner.",
        locationName: "Central Library, 3rd Floor",
        lostAt: new Date("2025-12-15T14:30:00Z"),
        status: "OPEN",
        rewardAmount: 50.0,
        images: ["https://images.unsplash.com/photo-1517336714731-489689fd1ca8?w=400"],
        tags: ["laptop", "electronics", "macbook"],
        brand: "Apple",
        color: "Silver",
        visibility: "PUBLIC",
        createdAt: new Date(),
        updatedAt: new Date(),
        _class: "com.lostcity.model.LostReport"
      },
      {
        _id: new ObjectId(),
        reportedBy: { $ref: "users", $id: john._id },
        title: "Car Keys with Blue Keychain",
        category: "KEYS",
        description: "Toyota car keys with a blue leather keychain and two house keys attached",
        locationName: "Student Center Cafeteria",
        lostAt: new Date("2025-12-16T10:00:00Z"),
        status: "OPEN",
        rewardAmount: 20.0,
        images: ["https://images.unsplash.com/photo-1582139329536-e7284fece509?w=400"],
        tags: ["keys", "car", "toyota"],
        color: "Blue",
        visibility: "PUBLIC",
        createdAt: new Date(),
        updatedAt: new Date(),
        _class: "com.lostcity.model.LostReport"
      },
      {
        _id: new ObjectId(),
        reportedBy: { $ref: "users", $id: mike._id },
        title: "Brown Leather Wallet",
        category: "WALLET",
        description: "Contains student ID, two credit cards, and some cash",
        locationName: "Gym Locker Room",
        lostAt: new Date("2025-12-14T18:45:00Z"),
        status: "OPEN",
        rewardAmount: 30.0,
        images: ["https://images.unsplash.com/photo-1627123424574-724758594e93?w=400"],
        tags: ["wallet", "leather", "personal"],
        brand: "Fossil",
        color: "Brown",
        visibility: "PUBLIC",
        createdAt: new Date(),
        updatedAt: new Date(),
        _class: "com.lostcity.model.LostReport"
      },
      {
        _id: new ObjectId(),
        reportedBy: { $ref: "users", $id: sarah._id },
        title: "Red Backpack",
        category: "BAG",
        description: "North Face red backpack with laptop compartment. Has a small teddy bear keychain.",
        locationName: "Bus Stop near Campus",
        lostAt: new Date("2025-12-13T08:15:00Z"),
        status: "MATCHED",
        rewardAmount: 25.0,
        images: ["https://images.unsplash.com/photo-1553062407-98eeb64c6a62?w=400"],
        tags: ["backpack", "bag", "northface"],
        brand: "North Face",
        color: "Red",
        visibility: "PUBLIC",
        createdAt: new Date(),
        updatedAt: new Date(),
        _class: "com.lostcity.model.LostReport"
      }
    ];
    
    await db.collection('lost_reports').insertMany(lostReports);
    console.log(`✅ Inserted ${lostReports.length} lost reports\n`);
    
    // ===== INSERT FOUND REPORTS =====
    console.log("📦 Inserting found reports...");
    const foundReports = [
      {
        _id: new ObjectId(),
        reportedBy: { $ref: "users", $id: sarah._id },
        title: "iPhone 13 Pro",
        category: "ELECTRONICS",
        description: "Black iPhone with a purple case. Lock screen shows a dog photo.",
        locationName: "Science Building, Room 204",
        foundAt: new Date("2025-12-16T11:00:00Z"),
        status: "OPEN",
        images: ["https://images.unsplash.com/photo-1592286927505-c0d0eb5f428b?w=400"],
        tags: ["phone", "iphone", "electronics"],
        color: "Black",
        brand: "Apple",
        foundCondition: "GOOD",
        createdAt: new Date(),
        updatedAt: new Date(),
        _class: "com.lostcity.model.FoundReport"
      },
      {
        _id: new ObjectId(),
        reportedBy: { $ref: "users", $id: sarah._id },
        title: "Blue Water Bottle",
        category: "OTHER",
        description: "Hydro Flask blue water bottle with stickers",
        locationName: "Basketball Court",
        foundAt: new Date("2025-12-15T16:30:00Z"),
        status: "OPEN",
        images: ["https://images.unsplash.com/photo-1602143407151-7111542de6e8?w=400"],
        tags: ["waterbottle", "hydroflask"],
        color: "Blue",
        brand: "Hydro Flask",
        foundCondition: "GOOD",
        createdAt: new Date(),
        updatedAt: new Date(),
        _class: "com.lostcity.model.FoundReport"
      },
      {
        _id: new ObjectId(),
        reportedBy: { $ref: "users", $id: mike._id },
        title: "Textbook - Calculus II",
        category: "DOCUMENTS",
        description: "Calculus textbook with name 'Emma Wilson' written inside",
        locationName: "Mathematics Building, 2nd Floor",
        foundAt: new Date("2025-12-14T13:20:00Z"),
        status: "MATCHED",
        images: ["https://images.unsplash.com/photo-1544716278-ca5e3f4abd8c?w=400"],
        tags: ["textbook", "book", "calculus"],
        foundCondition: "GOOD",
        createdAt: new Date(),
        updatedAt: new Date(),
        _class: "com.lostcity.model.FoundReport"
      },
      {
        _id: new ObjectId(),
        reportedBy: { $ref: "users", $id: john._id },
        title: "Prescription Glasses",
        category: "ACCESSORIES",
        description: "Black frame prescription glasses in a brown case",
        locationName: "Library Study Room 3",
        foundAt: new Date("2025-12-13T19:00:00Z"),
        status: "OPEN",
        images: ["https://images.unsplash.com/photo-1574258495973-f010dfbb5371?w=400"],
        tags: ["glasses", "prescription", "accessories"],
        color: "Black",
        foundCondition: "GOOD",
        createdAt: new Date(),
        updatedAt: new Date(),
        _class: "com.lostcity.model.FoundReport"
      },
      {
        _id: new ObjectId(),
        reportedBy: { $ref: "users", $id: sarah._id },
        title: "Gold Ring",
        category: "JEWELRY",
        description: "Gold ring with small diamond, possibly engagement ring",
        locationName: "Women's Restroom, Engineering Building",
        foundAt: new Date("2025-12-12T14:00:00Z"),
        status: "OPEN",
        images: ["https://images.unsplash.com/photo-1605100804763-247f67b3557e?w=400"],
        tags: ["ring", "jewelry", "gold", "diamond"],
        color: "Gold",
        foundCondition: "EXCELLENT",
        createdAt: new Date(),
        updatedAt: new Date(),
        _class: "com.lostcity.model.FoundReport"
      }
    ];
    
    await db.collection('found_reports').insertMany(foundReports);
    console.log(`✅ Inserted ${foundReports.length} found reports\n`);
    
    // ===== SUMMARY =====
    console.log("📊 Database Seeding Summary:");
    console.log("============================");
    console.log(`✅ Users: ${users.length}`);
    console.log(`✅ Lost Reports: ${lostReports.length}`);
    console.log(`✅ Found Reports: ${foundReports.length}`);
    console.log("\n🎉 Database seeding completed successfully!");
    
  } catch (error) {
    console.error("❌ Error seeding database:", error);
    process.exit(1);
  } finally {
    await client.close();
    console.log("\n🔌 Disconnected from MongoDB");
  }
}

// Run the seeding
seedDatabase();
